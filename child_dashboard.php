...
// Access Control: Ensure only users logged in as 'Child' can enter
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'Child') {
    header("Location: login.php");
    exit();
}

// Fetch basic logged-in user profile info
$user_id_auth = $_SESSION['user_id'];
$username_auth = $_SESSION['username'];

// Retrieve alphanumeric Child custom ID from session mapping (e.g. C000000001)
$child_custom_id = $_SESSION['user_type_id'] ?? '';

// Define these at the top of your script after session checks
$child_id = $child_custom_id; // Using your mapped custom ID
//$sponsor_user_id = $sponsor_id; // From the existing lookup logic

$message = '';
$message_class = '';

// If custom ID isn't mapped properly, fetch it manually from the database
if (empty($child_custom_id)) {
    $u_stmt = $conn->prepare("SELECT user_type_id FROM users WHERE id = ?");
    $u_stmt->bind_param("i", $user_id_auth);
    $u_stmt->execute();
    $u_res = $u_stmt->get_result();
    if ($row = $u_res->fetch_assoc()) {
        $child_custom_id = $row['user_type_id'];
        $_SESSION['user_type_id'] = $child_custom_id;
    }
    $u_stmt->close();
}

// 1. VERIFY CHILD ACCOUNT Record EXISTENCE (Kept basic check, but decoupled from sponsor block)
$child_found = false;
if (!empty($child_custom_id)) {
    $c_stmt = $conn->prepare("SELECT * FROM child WHERE user_id = ?"); 
    $c_stmt->bind_param("s", $child_custom_id);
    $c_stmt->execute();
...
// 2. FETCH CONNECTED SPONSOR PROFILE FROM RELATIONSHIP COUPLING TABLE
if (!empty($child_custom_id)) {
    $sp_stmt = $conn->prepare("SELECT s.id,s.user_id, s.first_name, s.last_name FROM child_sponsor_matches cs JOIN sponsors s ON cs.sponsor_user_id = s.id WHERE cs.child_id = ? AND cs.match_status = 'Active' LIMIT 1");
    $sp_stmt->bind_param("s", $child_custom_id);
    $sp_stmt->execute();
    $sp_res = $sp_stmt->get_result();
    if ($sp_row = $sp_res->fetch_assoc()) {
        $sponsor_id = $sp_row['id'];
        $sponsor_name = $sp_row['first_name'] . ' ' . $sp_row['last_name'];
...
$sponsor_letters = [];
$sponsor_stmt = $conn->prepare("SELECT * FROM letters WHERE child_id = ? AND sender_role = 'Child' ORDER BY created_at DESC");
...
$child_letters = [];
$child_stmt = $conn->prepare("SELECT * FROM letters WHERE child_id = ? AND sender_role = 'Sponsor' AND status = 'Approved' ORDER BY created_at DESC");
...
<head>
    ...
    <title>Child Letter Center</title>
    <style>
        body { font-family: 'Segoe UI', Arial, sans-serif; margin: 0; background-color: #f4f6f9; color: #333; }
        .navbar { background-color: #79520a; color: white; padding: 15px 30px; display: flex; justify-content: space-between; align-items: center; }
        .navbar h1 { margin: 0; font-size: 20px; }
        .navbar a { color: white; text-decoration: none; font-size: 14px; font-weight: bold; }
        .btn-logout { background-color: #1d7208; padding: 8px 15px; border-radius: 4px; font-weight: bold; transition: 0.2s; }
        .btn-logout:hover { background-color: #1d7208; }
        
        .container { max-width: 1200px; margin: 30px auto; padding: 0 20px; }
        .back-link { display: inline-block; margin-bottom: 20px; color: #79520a; text-decoration: none; font-weight: bold; }
        ...
        .card-title { font-size: 18px; font-weight: bold; color: #79520a; margin-bottom: 20px; border-bottom: 2px solid #edf2f7; padding-bottom: 8px; }
        ...
        .btn-submit { width: 100%; padding: 12px; background-color: #79520a; border: none; color: white; font-size: 15px; font-weight: bold; border-radius: 4px; cursor: pointer; margin-top: 15px; }
        ...
        .btn-action { padding: 6px 12px; background: #79520a; color: white; text-decoration: none; border-radius: 4px; font-size: 12px; font-weight: bold; border: none; cursor: pointer; }
...
<div class="navbar">
    <h1>🤝 Child Correspondence Portal</h1>
        ...
        <table class="history-table">
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        <td><strong><?php echo date("M d, Y", strtotime($let['created_at'])); ?></strong></td>
                        <td><span style="color: #b45309; font-weight: bold;">📥 Sponsor</span></td>
                        <td><?php echo htmlspecialchars(substr($let['letter_content'] ?? '', 0, 50)) . (strlen($let['letter_content'] ?? '') > 50 ? '...' : ''); ?></td>
                        <td>
                            <?php if (!empty($let['file_path'])): ?>
...
<div id="letterModal" class="modal-mask">
    <div class="modal-body">
        <h3 id="modalTitle" style="color: #79520a; margin-top: 0; border-bottom: 2px solid #edf2f7; padding-bottom: 8px;">Document Details</h3>
        <p><strong>System Record Meta-State:</strong> <span id="modalStatus" class="badge">Pending</span></p>
        <div id="modalCommentBox" style="display: none; margin-bottom: 15px;"></div>
